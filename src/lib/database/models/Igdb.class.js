...
class Igdb {
	...
	static async count() {
		const { rows } = await pool.query("SELECT COUNT(*) FROM igdb");
	...
	static async exists(id) {
		const { rows } = await pool.query("SELECT 1 FROM igdb WHERE id = $1", [
			id,
		]);
	...
	static async getById(id) {
		...
		const { rows } = await pool.query(`SELECT * FROM igdb WHERE id = $1`, [
			id,
		]);
