...
class Movie {
  ...
  static async create(newMovie) {
    if (!newMovie?.name) {
      throw Object.assign(new Error("Movie 'name' is required"), { httpStatusCode: 400});
    }

    if (!newMovie?.year) {
      throw Object.assign(new Error("Movie 'year' is required"), { httpStatusCode: 400});
    };
  ...
  static async getById(id) {
    ...
    if ( !rows.length ) {
      throw Object.assign(new Error(`Movie does not exist with id: ${id}`), { code: 400, httpStatusCode: 400});
    }
