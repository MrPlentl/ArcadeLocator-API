...
class Show {
  ...
  static async exists(id) {
    const { rows } = await pool.query("SELECT 1 FROM show WHERE id = $1", [id]);
  ...
  static async create(newShow) {
    if (!newShow?.name) {
      throw Object.assign(new Error("Show 'name' is required"), { httpStatusCode: 400});
    }

    if (!newShow?.year) {
      throw Object.assign(new Error("Show 'year' is required"), { httpStatusCode: 400});
    };
  ...
  static async getAll(orderBy) {
    ...
    const { rows } = await pool.query(`SELECT * FROM show ORDER BY ${orderBy}`);
    return rows;
  }
  ...
  static async getById(id) {
    ...
    if ( !rows.length ) {
      throw Object.assign(new Error(`Shows does not exist with id: ${id}`), { code: 400, httpStatusCode: 400});
    }
  ...
  static async update(id, data) {
    ...
    const invalidKeys = keys.filter(key => !VALID_SHOW_FIELDS.includes(key));
  ...
  static async delete(id) {
    ...
    const { rows } = await pool.query("DELETE FROM show WHERE id = $1 RETURNING *", [id]);
    return rows[0] || null;
  }
}
