...
import Show from "../../../database/models/Show.class.js";
...
export async function createNewShow(req) {
  ...
  try {
    ...
    let msg = 'An error occured while creating a new Show. Check the details and please try again. Contact Support if the problem presists.';
    if (error.code == 23505){
      msg = 'Duplicate Show Detected: A show already exists with that name and year. Check the details and please try again. Contact Support if the problem presists.';
    };
...
export async function fetchAllShows(req) {
    ...
    try {
      ...
      const msg = 'An error occured while fetching shows. Please try again later and contact Support if the problem presists.';
      return formatErrorResponse(sqlError(error, msg));
    }
};
