...
process.argv.forEach((argValue) => {
	if (ctr > 1) {
		switch (argValue) {
			...
			default:
				throw new Error(
					`Unknown Argument Detected => "` + argValue + `"`,
				);
		}
	}
	ctr++;
...
const authenticateRateLimiter = rateLimit({
	...
	message:
		"Rate Limit Reached, too many requests to authenticate apikey with server. Please try again in 30 minutes. WARNING: Do not continuously authenticate your Api Key to get a new Auth token. Auth Tokens are valid for 60 minutes.",
	standardHeaders: "draft-8", // draft-6: `RateLimit-*` headers; draft-7 & draft-8: combined `RateLimit` header
});
